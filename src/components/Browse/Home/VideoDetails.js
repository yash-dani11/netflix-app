...
const VideoDetails = ({details,category}) => {
  const dispatch = useDispatch();
  const {title,desc,id} = details;
  const openModal = ()=>{
  ...
  return (
    <div className='h-full text-white relative bg-opacity-65 z-10 text-center md:text-left'>
        ...
        <div>
            ...
            <button className='bg-gray-500 w-24 px-2 py-2 md:px-4 bg-opacity-60 font-bold rounded-md md:w-32 hover:bg-opacity-40' onClick={openModal}> More Info</button>
        </div>
    </div>
    </div>
